...
def chat():
    try:
        ...
        # --- 1. VERIFICAMOS EL CEREBRO (GEMINI) ---
        ...
        if res.status_code != 200:
            return jsonify({"respuesta": f"ERROR DE GEMINI: {res.text}", "audio": None})
            
        txt = res.json()['candidates'][0]['content']['parts'][0]['text']

        # --- 2. VERIFICAMOS LAS CUERDAS VOCALES (ELEVENLABS) ---
        ...
        if elevenlabs_api_key:
            ...
            if tts_res.status_code == 200:
                ...
                txt += f" [Error de Voz ElevenLabs: {tts_res.text}]"
